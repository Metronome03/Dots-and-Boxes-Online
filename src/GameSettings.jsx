...
{
    const {user}=useContext(UserContext);
    const {roomID,setRoom}=useContext(RoomContext);
    ...
    return (
        <div id="player-card" className="basis-3/12 w-full h-full flex flex-col justify-evenly items-center">
            ...
            <div id="admin-player-button" className="basis-2/12 md:basis-1/12 w-3/12 md:w-6/12 flex flex-row justify-end">
                <button id="remove-button" className="rounded-full basis-1/12 md:basis-2/12 h-full md:h-9/12 bg-red-800">
...
function PlayersDisplay()
{
    ...
    return (
        <div id="color-selector" className="basis-8/12 w-full flex flex-col md:flex-row justify-evenly items-center">
            {
...
function AdminView()
{
    ...
    const {roomID,gameReadyStatus}=useContext(RoomContext);
    ...
    return (
        <div id="view" className="basis-10/12 w-full text-xl font-bold flex flex-col justify-evenly items-center">
            ...
            <div id="control-buttons" className="basis-1/12 md:basis-2/12 w-10/12 sm:w-full flex flex-col sm:flex-row justify-evenly items-center">
                <StartButton />
                <button id="leave-game" onClick={()=>{console.log(room)}} className="basis-5/12 w-full h-full input-border hover:bg-black hover:text-white">Leave Game</button>
            </div>
        </div>
    );
}
